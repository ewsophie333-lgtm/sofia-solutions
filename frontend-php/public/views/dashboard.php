...
<main class="app-shell readdy-dashboard">
    ...
    <aside class="sidebar">
        ...
        <div
            style="margin-top:auto;padding:24px 20px;border-top:1px solid rgba(255,255,255,0.05); text-align:center; color:var(--text-muted); font-size:0.7rem;">
    ...
    <section class="content">
        ...
        <header class="panel-header"
            ...
            <div>
                ...
                <h1 style="margin:8px 0 0;">¡Bienvenido, <span id="welcome-user-name"
                        style="color:var(--primary);">...</span>!</h1>
                <p style="color:var(--text-muted); font-size:0.9rem; margin-top:4px;">Tu infraestructura está bajo
                    vigilancia activa por el equipo SOC.</p>
            </div>
            <div style="display:flex; align-items:center; gap:24px;">
                <!-- Minimalist SOS Button -->
                <button onclick="openSOS()"
                    style="background:rgba(220,38,38,0.08); color:#fca5a5; border:1px solid rgba(220,38,38,0.3); padding:8px 16px; border-radius:10px; font-weight:700; cursor:pointer; display:flex; align-items:center; gap:8px; transition:all 0.3s;"
                ...
                <div
                    style="display:flex;gap:4px;background:rgba(255,255,255,0.03);padding:4px;border-radius:10px;border:1px solid rgba(255,255,255,0.08);">
                    <button onclick="setLang('es')" id="btn-es" class="btn"
                        style="min-height:30px; min-width:40px; padding:0 8px; font-size:0.7rem; border-radius:6px; background:rgba(139,92,246,0.15); color:#fff; border:1px solid rgba(139,92,246,0.3);">ES</button>
                    <button onclick="setLang('en')" id="btn-en" class="btn"
                        style="min-height:30px; min-width:40px; padding:0 8px; font-size:0.7rem; border-radius:6px; background:transparent; color:var(--text-muted); border:none;">EN</button>
                </div>
            </div>
        </header>
        ...
        <section class="planes-grid" style="grid-template-columns:repeat(4,1fr);gap:20px;margin-bottom:36px;">
            <div class="kpi-card" data-tone="ok" id="kpi-assets"><span class="meta-label">Activos
                    Protegidos</span><strong>—</strong>
                <div class="tone-bar"></div>
            </div>
            <div class="kpi-card" data-tone="ok" id="kpi-uptime"><span class="meta-label">Disponibilidad
                    (SLA)</span><strong>99.9%</strong>
                <div class="tone-bar"></div>
            </div>
            <div class="kpi-card" data-tone="warn" id="kpi-blocked"><span class="meta-label">Amenazas
                    Bloqueadas</span><strong>—</strong>
                <div class="tone-bar"></div>
            </div>
            <div class="kpi-card" data-tone="ok" id="kpi-tickets"><span class="meta-label">Soporte
                    Activo</span><strong>—</strong>
                <div class="tone-bar"></div>
            </div>
        </section>
        ...
        <section class="planes-grid" style="grid-template-columns: 2fr 1fr; gap:32px; margin-bottom:40px;">
            <article class="panel" style="padding:24px;">
                <div class="panel-heading"
                    style="margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;">
                    <div><span class="eyebrow">Infraestructura</span>
                        <h2>Telemetría de Red y Rendimiento</h2>
                    </div>
                    <span id="server-status-dot"
                        style="display:inline-flex;align-items:center;gap:8px;font-size:0.75rem;font-weight:700;padding:6px 14px;border-radius:20px;background:rgba(34,197,94,0.1);color:#22c55e;border:1px solid rgba(34,197,94,0.2);">
                        OPERATIVO</span>
                </div>
            ...
            <article class="panel" style="padding:24px;">
                ...
                <div style="display:grid; gap:16px;">
                    <div
                        style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);border-radius:12px;padding:16px;display:flex;justify-content:space-between;align-items:center;">
                        <div>
                            <div style="font-size:0.72rem;color:var(--text-muted);">API Backend</div>
                            <div id="srv-api" style="font-weight:700;">—</div>
                        </div>
                    ...
                    <div
                        style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);border-radius:12px;padding:16px;display:flex;justify-content:space-between;align-items:center;">
                        <div>
                            <div style="font-size:0.72rem;color:var(--text-muted);">Base de Datos</div>
                            <div id="srv-db" style="font-weight:700;">—</div>
                        </div>
                        <div style="width:12px;height:12px;border-radius:50%;background:#22c55e;"></div>
                    </div>
                    <div
                        style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.05);border-radius:12px;padding:16px;display:flex;justify-content:space-between;align-items:center;">
                        <div>
                            <div style="font-size:0.72rem;color:var(--text-muted);">SOC Gateway</div>
                            <div id="srv-soc" style="font-weight:700;">—</div>
                        </div>
                        <div style="width:12px;height:12px;border-radius:50%;background:#22c55e;"></div>
                    </div>
                </div>
        ...
        <section style="margin-bottom:40px;">
            <article class="panel" style="padding:32px;">
                <div class="panel-heading"
                    ...
                    <div><span class="eyebrow">Soporte Técnico</span>
                        <h2>Gestión de Casos e Incidentes</h2>
                    </div>
                    <button class="btn btn-primary" onclick="openNewTicket()">+ Abrir Nuevo Caso</button>
                </div>

                <div
                    style="display:flex; gap:12px; margin-bottom:24px; border-bottom:1px solid rgba(255,255,255,0.05); padding-bottom:12px;">
                    <button onclick="filterTickets('all')" class="ticket-tab active"
                        style="background:none; border:none; color:#fff; font-weight:700; cursor:pointer; font-size:0.9rem;">Todos</button>
                    <button onclick="filterTickets('OPEN')" class="ticket-tab"
                        style="background:none; border:none; color:var(--text-muted); cursor:pointer; font-size:0.9rem;">Abiertos</button>
                    <button onclick="filterTickets('IN_PROGRESS')" class="ticket-tab"
                        style="background:none; border:none; color:var(--text-muted); cursor:pointer; font-size:0.9rem;">En
                        Proceso</button>
                    <button onclick="filterTickets('CLOSED')" class="ticket-tab"
                        style="background:none; border:none; color:var(--text-muted); cursor:pointer; font-size:0.9rem;">Resueltos</button>
                </div>

                <div id="tickets-list-container">
                    <div id="tickets-list">
                        <div style="padding:40px;text-align:center;color:var(--text-muted);">Cargando tus tickets...
                        </div>
                    </div>
                </div>
            </article>
        ...
        <section class="planes-grid" style="grid-template-columns:1.5fr 1fr;gap:32px;margin-bottom:40px;">
            <article class="panel" style="padding:24px;">
                <div class="panel-heading"
                    style="margin-bottom:24px; display:flex; justify-content:space-between; align-items:center;">
                    <div><span class="eyebrow">Suscripción</span>
                        <h2>Estado de Cuenta</h2>
                    </div>
                    <button class="btn btn-primary" onclick="toggleUpgrades()" style="font-size:0.75rem;">Mejorar
                        Suscripción ↑</button>
                </div>
                <div id="active-plan-display">
                    <!-- Dynamic active plan card -->
                </div>
                <div id="upgrade-catalog"
                    style="display:none; margin-top:24px; padding-top:24px; border-top:1px solid rgba(255,255,255,0.05); animation: slideDown 0.4s ease;">
                    <div class="eyebrow" style="margin-bottom:16px;">Planes Disponibles para Upgrade</div>
                    <div style="display:grid; grid-template-columns: 1fr 1fr; gap:16px;">
                        <div onclick="openPayment('Business Premium', 1500)"
                            style="cursor:pointer; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.08); border-radius:12px; padding:16px; transition:all 0.3s;"
                        ...
                        <div onclick="openPayment('Enterprise SOC', 4200)"
                            style="cursor:pointer; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.08); border-radius:12px; padding:16px; transition:all 0.3s;"
                            onmouseover="this.style.borderColor='var(--primary)'"
                            onmouseout="this.style.borderColor='rgba(255,255,255,0.08)'">
            ...
            <article class="panel" style="padding:24px;">
                ...
                <div class="stack-list">
                    <div class="stack-item" style="display:flex;justify-content:space-between;align-items:center;">
                        <div><strong>Factura Abril 2026</strong>
                            <div style="font-size:0.75rem;color:var(--text-muted);">PDF · 1.2 MB</div>
                        </div>
                        <a href="/download.php?file=invoice_1023.pdf" class="btn btn-outline btn-sm">Descargar</a>
                    </div>
                    <div class="stack-item" style="display:flex;justify-content:space-between;align-items:center;">
                        <div><strong>Informe de Seguridad Mar 2026</strong>
                            <div style="font-size:0.75rem;color:var(--text-muted);">PDF · 3.4 MB</div>
                        </div>
...
<!-- SOS Emergency Modal -->
<div id="sos-modal"
    style="display:none; position:fixed; inset:0; background:rgba(0,0,0,0.9); backdrop-filter:blur(10px); z-index:10000; align-items:center; justify-content:center; padding:20px;">
    <div class="panel"
        style="width:min(450px, 100%); border:2px solid #ef4444; box-shadow:0 0 30px rgba(239,68,68,0.3);">
        <div style="text-align:center; margin-bottom:24px;">
            <div
                style="background:rgba(239,68,68,0.1); width:64px; height:64px; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 16px; border:1px solid rgba(239,68,68,0.3);">
            ...
            <p style="color:var(--text-muted); font-size:0.9rem; margin-top:8px;">Envía una notificación prioritaria al
                equipo SOC.</p>
        </div>
        <div style="display:grid; gap:12px;">
            <label style="font-size:0.8rem; font-weight:700; color:var(--text-soft);">Razón de la emergencia:</label>
            <textarea id="sos-reason"
                style="width:100%; height:100px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:10px; color:#fff; padding:12px; font-family:inherit; font-size:0.9rem; resize:none;"
                placeholder="Ej: He detectado un acceso no autorizado en el servidor de producción..."></textarea>
...
<div class="pay-modal" id="pay-modal">
    <div class="pay-card">
        <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:24px;">
            <div><span class="eyebrow">Checkout Seguro</span>
                <h2 id="pay-plan-name" style="margin:4px 0;font-size:1.4rem;">Plan</h2>
            </div>
            <button onclick="closePayment()"
                style="background:none;border:none;color:#fff;font-size:1.3rem;cursor:pointer;">&times;</button>
        </div>
        <div
            style="background:rgba(6,182,212,0.05);border:1px solid rgba(6,182,212,0.15);border-radius:12px;padding:16px;margin-bottom:20px;">
            <div style="display:flex;justify-content:space-between;"><span style="color:var(--text-muted);"
                    data-i18n="pay_subtotal">Subtotal mensual</span><strong id="pay-amount"></strong></div>
            <div style="display:flex;justify-content:space-between;margin-top:8px;"><span
                    style="color:var(--text-muted);" data-i18n="pay_tax">IVA (21%)</span><strong id="pay-tax"></strong>
            </div>
            <div
                style="display:flex;justify-content:space-between;margin-top:12px;padding-top:12px;border-top:1px solid rgba(255,255,255,0.05);">
                <span style="font-weight:700;" data-i18n="pay_total">Total</span><strong id="pay-total"
                    style="color:var(--primary);font-size:1.1rem;"></strong></div>
        </div>
        <div style="display:grid;gap:12px;margin-bottom:20px;">
            <div><label style="font-size:0.72rem;color:var(--text-muted);display:block;margin-bottom:5px;"
                    data-i18n="pay_card">Número de tarjeta</label><input
                    style="width:100%;padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;color:#fff;box-sizing:border-box;"
                    placeholder="4532 •••• •••• 8910"></div>
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                <div><label style="font-size:0.72rem;color:var(--text-muted);display:block;margin-bottom:5px;"
                        data-i18n="pay_exp">Caducidad</label><input
                        style="width:100%;padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;color:#fff;box-sizing:border-box;"
                        placeholder="MM/AA"></div>
                <div><label
                        style="font-size:0.72rem;color:var(--text-muted);display:block;margin-bottom:5px;">CVV</label><input
                        style="width:100%;padding:12px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.1);border-radius:10px;color:#fff;box-sizing:border-box;"
                        placeholder="•••"></div>
            </div>
        </div>
        <button onclick="processPayment()" class="btn btn-primary btn-block" data-i18n="pay_btn">Confirmar Pago Seguro
            🔒</button>
        <p style="font-size:0.7rem;color:var(--text-muted);text-align:center;margin-top:12px;" data-i18n="pay_note">Pago
            procesado mediante cifrado TLS 1.3. Puedes cancelar en cualquier momento.</p>
    </div>
</div>
